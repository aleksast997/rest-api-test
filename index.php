...
<form action="">
    <div id="odabir_tabele">
        <input type="radio" name="odabir_tabele" id="radio_kategorija" vlaue="kategorija">
        <label for="radio_kategorija">kategorija</label>
        <input type="radio" name="odabir_tabele" id="radio_novosti" vlaue="novosti">
    ...
    <div id="novosti_post">
        ...
        <br>
        <textarea name="tekst_novosti" id="tekst_novosti" cols="30" rows="10" placeholder="Unestite tekst"></textarea>
        <br>
    ...
    <div id="kategorije_post">
        <input type="text" name="kategorija_naziv" id="kategorija_naziv" placeholder="Unesite naziv nove kateogorije">
    </div>

    <div id="brisanje_reda">
        <input type="text" name="brisanje" id="brisanje" placeholder="Unesite id koji zelite da obirsete">
    </div>
    ...
    function posaljiZahtev(){
        if($("input[name=odabir_tabele]:checked").length!=0 && $("input[name=http_zahtev]:checked").length!=0){
            ...
            switch($("input[name=http_zahtev]:checked")[0].id){
                case "get":
                    if(tabela=="radio_novosti"){
                        $.getJSON("http://localhost/rest-2/api/novosti", function(data){
                            document.getElementById("get_odgovor").innerHTML = JSON.stringify(data,null,2);
                        });
                    }else{
                        $.getJSON("http://localhost/rest-2/api/kategorije", function(data){
                            document.getElementById("get_odgovor").innerHTML = JSON.stringify(data,null,2);
                        });
                    }
                    break;
                case "post":
                    if(tabela=="radio_novosti"){
                        ...
                        console.log(values);
                        $.post("http://localhost/rest-2/api/novosti", JSON.stringify(values),function(data){
                            alert("Odgovor servera: "+data['poruka']);
                        });
                    }else{
                        var values = {
                        ...
                        console.log(values);
                        $.post("http://localhost/rest-2/api/kategorije", JSON.stringify(values),function(data){
                            alert("Odgovor servera: "+data['poruka']);
                        });
                    }
                    break;
                    ...
                    if(tabela=="radio_novosti"){
                        ...
                        var values={
                            "kategorija": $("input[name=kategoirja_naziv_put]").val() 
                        };
                        $.ajax({
                            url:"http://localhost/rest-2/api/kategorije/"+parseInt($("#kategorija_id").val()),
                            type:"PUT",
                            data:JSON.stringify(values)
                        }).done(function(data){
                            alert("Odgovor sa servera> "+data["poruka"]);
